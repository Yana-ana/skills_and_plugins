...
- `edges.source_ref` 和 `edges.target_ref` 必须引用已存在的 `nodes.ref_id`。
- `properties.owner_ref` 必须引用已存在的 `nodes.ref_id` 或 `edges.ref_id`。
- `evidence.target_ref` 必须引用已存在的 `nodes.ref_id` 或 `edges.ref_id`。
...
- 当 `evidence.source_type` 为 `summary` 时，`source_ref` 必须包含 `conversation_id` 和 `date`，且使用本次查询到的真实值。
