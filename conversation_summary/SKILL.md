...
description: 【对话总结 skill - 通用】基于 conversation_id + users_id，通过插件工具 query_database 查询 conversation 和 conversation_item 数据，组装对话原文，识别场景，并返回严格 JSON；顶层必须包含 title、content、scene、knowledge_graph 四个字段。
---
...
5. 生成最终严格 JSON，顶层必须且只能包含 `title`、`content`、`scene`、`knowledge_graph` 四个字段。
...
必须调用插件工具 `query_database` 两次。因为调用方已经显式提供了 `users_id`，这里不要额外追加 `claw_id` 条件，所以 `UseClawInstanceFilter` 必须传布尔值 `false`，不能传字符串 `"False"` 或 `"false"`。
...
要求：
- 如果返回 0 条，说明对话不存在或不属于该用户，不要继续查 item；仍然必须返回严格 JSON：`title` 写“对话不存在”，`content` 说明原因，`scene` 写 `other`，`knowledge_graph` 返回四个空数组。
...
- 如果 item 返回 0 条，不要输出自然语言错误；仍然返回严格 JSON，`content` 说明该对话没有可总结内容，`knowledge_graph` 返回四个空数组。
...
- 必须直接输出 JSON 对象本身。
- 不要输出 Markdown 代码块，例如 ```json。
- 不要在 JSON 前后输出任何解释文字、`true`、`OK`、工具调用过程、状态说明或自然语言前缀。
- 不要只返回 `{ "title": ..., "content": ..., "scene": ... }` 三字段结果。
- `knowledge_graph` 不能省略；没有图谱数据时返回四个空数组。
- `content` 可以用 Markdown 写总结正文，但不能用正文里的“知识图谱”段落替代结构化 `knowledge_graph` 字段。
...
- 最终回复必须是严格合法 JSON。
- 顶层必须且只能包含 `title`、`content`、`scene`、`knowledge_graph`。
- `knowledge_graph` 必须且只能包含 `nodes`、`edges`、`properties`、`evidence`。
- 如果没有可抽取图谱数据，`nodes`、`edges`、`properties`、`evidence` 返回空数组。
- 不要返回 Markdown fenced code block，不要返回 `true```json`、`json\n{...}` 或任何非 JSON 包装。
